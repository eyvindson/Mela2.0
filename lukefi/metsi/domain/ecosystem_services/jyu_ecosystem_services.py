...
def _species_metrics(stand: "ForestStand") -> dict[str, float]:
    ...
    for i in range(tree_count):
        ...
        vol = stems * float(volumes[i] if volumes is not None else 0.0)
...
def calculate_jyu_ecosystem_services_fn(input_: "ForestStand", /, **_operation_parameters) -> OpTuple["ForestStand"]:
    ...
    age = float(getattr(input_, "dominant_storey_age", 0.0) or 0.0)
    ...
    print(metrics["vol_pine"], metrics["vol_spruce"],metrics["vol_birch"])
